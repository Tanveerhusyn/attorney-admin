...
export async function getFAQByCityId(cityId: string): Promise<{
  success?: boolean;
  data?: FAQ[];
  error?: ErrorResponse;
}> {
...
export async function generateFAQs(cityName: string): Promise<{
  success?: boolean;
  data?: FAQ[];
  error?: ErrorResponse;
}> {
  try {
    const response = await fetch(`${API_URL}/faqs/generate`,{
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export async function saveFAQs({faqs,cityId}:{faqs:FAQ[],cityId:string}): Promise<{
  success?: boolean;
  data?: FAQ[];
  error?: ErrorResponse;
}> {
  try {
    const payload = {faqs,cityId}
    const response = await fetch(`${API_URL}/faqs/addMany`,{
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
